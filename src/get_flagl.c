...
/*   Updated: 2020/11/11 10:50:14 by cgamora          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			set_device(t_info *tmp_list, t_flagl *inform)
{
	int tmp;
...
void		set_digit(t_flagl *inform)
{
	...
	if (inform->device)
		inform->st_size = MAX(2 + inform->major + inform->minor, inform->st_size);
}

t_flagl			init_information(void)
{
	t_flagl		inf;
